/****************************************************************************
** Meta object code from reading C++ file 'tilesetdock.h'
**
** Created: Wed May 11 10:52:21 2011
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../tiled/src/tiled/tilesetdock.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'tilesetdock.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Tiled__Internal__TilesetDock[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
      10,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: signature, parameters, type, tag, flags
      35,   30,   29,   29, 0x05,
      67,   61,   29,   29, 0x05,
     111,  105,   29,   29, 0x05,

 // slots: signature, parameters, type, tag, flags
     154,  140,   29,   29, 0x08,
     186,   29,   29,   29, 0x08,
     215,  207,   29,   29, 0x08,
     240,  207,   29,   29, 0x08,
     273,  265,   29,   29, 0x08,
     301,  295,   29,   29, 0x08,
     320,  265,   29,   29, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_Tiled__Internal__TilesetDock[] = {
    "Tiled::Internal::TilesetDock\0\0tile\0"
    "currentTileChanged(Tile*)\0tiles\0"
    "currentTilesChanged(const TileLayer*)\0"
    "paths\0tilesetsDropped(QStringList)\0"
    "index,tileset\0insertTilesetView(int,Tileset*)\0"
    "updateCurrentTiles()\0tileset\0"
    "tilesetChanged(Tileset*)\0"
    "tilesetRemoved(Tileset*)\0from,to\0"
    "tilesetMoved(int,int)\0index\0"
    "removeTileset(int)\0moveTileset(int,int)\0"
};

const QMetaObject Tiled::Internal::TilesetDock::staticMetaObject = {
    { &QDockWidget::staticMetaObject, qt_meta_stringdata_Tiled__Internal__TilesetDock,
      qt_meta_data_Tiled__Internal__TilesetDock, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Tiled::Internal::TilesetDock::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Tiled::Internal::TilesetDock::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Tiled::Internal::TilesetDock::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Tiled__Internal__TilesetDock))
        return static_cast<void*>(const_cast< TilesetDock*>(this));
    return QDockWidget::qt_metacast(_clname);
}

int Tiled::Internal::TilesetDock::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDockWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: currentTileChanged((*reinterpret_cast< Tile*(*)>(_a[1]))); break;
        case 1: currentTilesChanged((*reinterpret_cast< const TileLayer*(*)>(_a[1]))); break;
        case 2: tilesetsDropped((*reinterpret_cast< const QStringList(*)>(_a[1]))); break;
        case 3: insertTilesetView((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< Tileset*(*)>(_a[2]))); break;
        case 4: updateCurrentTiles(); break;
        case 5: tilesetChanged((*reinterpret_cast< Tileset*(*)>(_a[1]))); break;
        case 6: tilesetRemoved((*reinterpret_cast< Tileset*(*)>(_a[1]))); break;
        case 7: tilesetMoved((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 8: removeTileset((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 9: moveTileset((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        default: ;
        }
        _id -= 10;
    }
    return _id;
}

// SIGNAL 0
void Tiled::Internal::TilesetDock::currentTileChanged(Tile * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void Tiled::Internal::TilesetDock::currentTilesChanged(const TileLayer * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void Tiled::Internal::TilesetDock::tilesetsDropped(const QStringList & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_END_MOC_NAMESPACE
